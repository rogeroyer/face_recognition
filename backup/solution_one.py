...
class FaceRecognize(object):
    def __init__(self, num_round=10, learn_rate=0.001, shuffle=False, divide_rate=0.9, units=1024, early_stopping=5, store_model=False):
    ...
    def run(self):
        ...
        """The number of neural cell in full-connection layer"""
    ...
    def store_model(self, sess, epoch):
        ...
        saver.save(sess, "model/cnn_multi_classifier", global_step=epoch)     # global_step
    ...
    def load_model(self):
        if os.path.exists('model') is True:
            ...
            if len(dir_list) == 0:
                ...
                with tf.Session() as sess:
                    path = './model/cnn_multi_classifier-19.meta'
                    ...
                    test = self.predict()   # load test image

                    graph = tf.get_default_graph()
                    input_x = graph.get_tensor_by_name('input_x:0')
                    # logit = graph.get_tensor_by_name('dropout:0')

                    feed_dict = {input_x: test}
                    # result = sess.run(logit, feed_dict=feed_dict)
                    result = sess.run(input_x)
                    print(result)
...
if __name__ == '__main__':
    face = FaceRecognize(num_round=10, learn_rate=0.0005, shuffle=False, divide_rate=0.9, units=2019, early_stopping=10, store_model=True)
    face.run()

    # face.load_model()     # un-realization
